...
const Main = () => {
    ...
    useEffect(() => {
        if (choosedFilters.length > 0) {
            ...
            axios.post(process.env.REACT_APP_BACKEND_DOMAIN + "/api/v1/projects/filter_by_filter_types/?page=0&size=10", {
            ...
            axios.get(process.env.REACT_APP_BACKEND_DOMAIN + "/api/v1/projects/all/?page=0&size=10")
            .then(res => {
                setProjects(res.data)
                const showMore = document.getElementById('show_more')
    ...
    const loadMoreProjects = async (e: any) => {
        ...
        if (choosedFilters.length > 0) {
            ...
            await axios.post(process.env.REACT_APP_BACKEND_DOMAIN + projectUrl + `?page=${newPage}&size=10`, {
                filter_types: choosedFilters
            })
            .then(res => {
                ...
                <div className='project-filters-block'>
                    <div onClick={dropDownToggle} className='drop-down'>
                        ...
                        <svg viewBox="0 0 18 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M7 11.727H11V9.81792H7V11.727ZM0 0.272461V2.18155H18V0.272461H0ZM3 6.95428H15V5.04519H3V6.95428Z" fill="currentcolor"></path>
                        </svg>
                    </div>
                    <div onClick={e => e.stopPropagation()} className='drop-down-list'>
                        {
                            filters &&
                            filters.map((filter: any) => (
                                <div id={filter.id} onClick={changeChoosedFiltersList} className='drop-down-item' key={filter.id}>
                ...
                <div className="projects-container">
                    <span className="side-text rotated-text">PORTFOLIO PROJECTS</span>
                    {
                        ...
                        projects.map((el: ProjectType) => (
                            <Project key={el.id} name={el.name} image={el.preview_image} tags={el.tags} project_link={el.source_link} checkout_link={`project/${el.id}`} />
                        ))
                    }
                </div>
