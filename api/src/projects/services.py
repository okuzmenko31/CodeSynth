...
from ..core.utils.media_files import get_media_file_link, save_media_file
...
class ProjectFilterTypeService(BaseService):
    ...
    async def update_filter_type(
        ...
        async with self.uow:
            ...
            await self.uow.project_types.update_by_data(instance_id, dict(data))
            await self.uow.commit()
...
class ProjectTagService(BaseService):
    ...
    async def create_tag(
        ...
        async with self.uow:
            tag_id = await self.uow_repo.insert_by_data(
                {"name": tag_name, "img": await get_media_file_link(hashed_filename)}
            )
    ...
    async def update_tag(
        ...
        async with self.uow:
            ...
            await self.uow.project_tags.update_by_data(instance_id, update_data)
            await self.uow.commit()
            return ReturnData(result=await self.repository.get_return_schema(tag))

    @handle_errors
    async def get_available_project_tags(self, project_id):
        async with self.uow:
...
class ProjectService(ProjectTagService):
    ...
    async def create_project(self, data: ProjectSchema, image_file):
        hashed_filename = await save_media_file(image_file)
        async with self.uow:
    ...
    async def add_project_tags(
        ...
        return await self.edit_project_tags(
            instance_id,
            data,
