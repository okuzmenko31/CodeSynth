...
from ..core.utils.dataclasses import (
    ReturnData,
    ProjectRequestCreateData,
    ProjectRequestUpdateData,
)
from ..core.utils.service_utils import return_data_err_object_does_not_exist
from .models import ProjectRequest

from ..core.utils.media_files import save_media_file, get_media_file_link
...
async def order_by_name_others_case_whens(model):
    return ((model.name == "Others", 1), (model.name == "others", 1))
...
class ProjectRequestService(BaseService):
    ...
    @handle_errors
    async def create_project_request(self, data: ProjectRequestCreateData):
        ...
        async with self.uow:
            ...
            if data.ref_source_id is not None:
                ref_source = await self.uow.project_ref_sources.get_one_by_id(
                    data.ref_source_id
                )
            ...
            services_list = await ProjectServicesService(
                self.uow
            ).get_instances_list_by_ids(project_services_json)
            project_request: ProjectRequest = (
                await self.uow_repo.create_instance_by_data(asdict(data))
            )
            ...
            if data.technical_task is not None:
                ...
                project_request.technical_task = await get_media_file_link(
                    hashed_filename
                )
            ...
            return ReturnData(
                result=await self.repository.get_return_schema(project_request)
            )

    @handle_errors
    async def update_project_request(
        self, request_id: int, data: ProjectRequestUpdateData
    ):
        if data.technical_task is not None:
            hashed_filename = await save_media_file(data.technical_task)
        ...
        async with self.uow:
            ...
            if data.budget_id is not None:
                ...
                if budget is None:
                    return await return_data_err_object_does_not_exist("budget")
            if data.ref_source_id is not None:
                source = await self.uow.project_ref_sources.get_one_by_id(
                    data.ref_source_id
                )
                if source is None:
    ...
    async def edit_project_request_services(
        ...
        async with self.uow:
            services_lst = await ProjectServicesService(
                self.uow
            ).get_instances_list_by_ids(data.services)
    ...
    async def add_project_request_services(
        self, request_id: int, data: ProjectRequestServicesUpdateSchema
    ):
        return await self.edit_project_request_services(
            request_id, data, ModelRelatedListOperations.append
        )
    ...
    async def remove_project_request_services(
        ...
        return await self.edit_project_request_services(
            request_id, data, ModelRelatedListOperations.remove
        )
